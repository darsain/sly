(function(c,J){function ea(h,b){var C,g,q,t,fa,s,K;function v(a,ga,L){if(A&&(!ga||L)){var h=I(a);G?(a=k[h.centerItem].offCenter,D&&f.activate(h.centerItem,1)):a>d.min&&a<d.max&&(a=k[h.firstItem].offStart)}"slidee"===ga&&b.elasticBounds&&!L?a>d.max?a=d.max+(a-d.max)/6:a<d.min&&(a=d.min+(a-d.min)/6):a=H(a,d.min,d.max);C=+new Date;g=0;q=d.cur;t=a;fa=a-d.cur;K=(s=ga)&&!L;d.dest=a;K||f.cycle();c.extend(e,I(void 0));ba();U[0]&&E.page!==e.activePage&&(E.page=e.activePage,U.removeClass(b.activeClass).eq(e.activePage).addClass(b.activeClass));
a!==d.cur&&!V&&F()}function F(){V?((K?"slidee"===s:33>b.speed)?d.cur=t:K&&"handle"===s?d.cur+=(t-d.cur)*b.syncFactor:(g=Math.min(+new Date-C,b.speed),d.cur=q+fa*jQuery.easing[b.easing](g/b.speed,g,0,1,b.speed)),t===Math.round(d.cur)&&(K||g>=b.speed)?(d.cur=d.dest,V=0,s||n.trigger(r+":moveEnd",[d,p,e])):V=ca(F),n.trigger(r+":move",[d,p,e]),M?j[0].style[M]=(b.horizontal?"translateX":"translateY")+"("+-d.cur+"px)":j[0].style[b.horizontal?"left":"top"]=-Math.round(d.cur)+"px",y&&(u.cur=d.min===d.max?
0:(("handle"===s?d.dest:d.cur)-d.min)/(d.max-d.min)*u.max,u.cur=H(Math.round(u.cur),u.min,u.max),E.hPos!==u.cur&&(E.hPos=u.cur,M?y[0].style[M]=(b.horizontal?"translateX":"translateY")+"("+u.cur+"px)":y[0].style[b.horizontal?"left":"top"]=u.cur+"px"))):(V=ca(F),s||n.trigger(r+":moveStart",[d,p,e]))}function W(a){return X(a)?H(a,0,k.length-1):a===J?-1:p.index(a)}function N(a,b){return parseInt(a.css(b),10)}function I(a){a=X(a)?a:d.dest;for(var b={},e=D?0:z/2,c=0,h=w.length;c<h;c++){if(a>=d.max||c===
w.length-1){b.activePage=w.length-1;break}if(a<=w[c]+e){b.activePage=c;break}}if(A){for(var h=c=e=!1,f=0,j=k.length;f<j;f++){!1===e&&a<=k[f].offStart&&(e=f);!1===h&&a-k[f].size/2<=k[f].offCenter&&(h=f);if(f===k.length-1||!1===c&&a<k[f+1].offEnd)c=f;if(!1!==c)break}b.firstItem=X(e)?e:0;b.centerItem=X(h)?h:b.firstItem;b.lastItem=X(c)?c:b.centerItem}return b}function ba(){if(A){var a=0===e.activeItem,c=e.activeItem>=k.length-1,f=a?"first":c?"last":"middle";E.itemsButtonState!==f&&(E.itemsButtonState=
...
window.event;a.preventDefault?a.preventDefault():a.returnValue=!1;b&&(a.stopPropagation?a.stopPropagation():a.cancelBubble=!0)}var f=this,n=c(h),j=n.children().eq(0),z=0,l=0,d={cur:0,dest:0,max:0,min:0},Q=c(b.scrollBar).eq(0),y=Q.length?Q.children().eq(0):0,$=0,R=0,u={cur:0,max:0,min:0},S=c(b.pagesBar),U=0,w=[],ia="smart"===b.itemNav,D="forceCentered"===b.itemNav,G="centered"===b.itemNav||D,A="basic"===b.itemNav||ia||G||D,T=c(document),p=0,k=[],e={firstItem:0,lastItem:1,centerItem:1,activeItem:-1,
...
2-f/2):0)+L,l+=L);l+=f;!b.horizontal&&!x&&e&&(h&&0<a)&&(l-=Math.min(h,e));a===p.length-1&&(l+=m);k.push(j)});j[0].style[b.horizontal?"width":"height"]=l+"px";l-=a;d.min=s;d.max=D?k[k.length-1].offCenter:Math.max(l-z,0);e.activeItem>=k.length?f.activate(k.length-1):1===k.length&&p.eq(e.activeItem).addClass(b.activeClass)}c.extend(e,I(void 0));y&&(R=b.dynamicHandle?Math.round($*z/l):y[b.horizontal?"outerWidth":"outerHeight"](),b.dynamicHandle&&(R=H(R,b.minHandleSize,$),y[0].style[b.horizontal?"width":
"height"]=R+"px"),u.max=$-R);var q=0,g="",t=0;if(D)w=c.map(k,function(a){return a.offCenter});else for(;q-z<d.max;){var B=Math.min(d.max,q);w.push(B);q+=z;if(q>d.max&&A&&d.max-B<(k[k.length-1].size-a)/2){w[w.length-1]=d.max;break}}if(S[0]){for(a=0;a<w.length;a++)g+=b.pageBuilder(t++);U=c(g).appendTo(S.empty())}v(H(d.dest,d.min,d.max));e.pages=w.length;e.slideeSize=l;e.frameSize=z;e.sbSize=$;e.handleSize=R;n.trigger(r+":load",[c.extend({},d,{old:h}),p,e])};this.prev=function(){f.activate(e.activeItem-
1)};this.next=function(){f.activate(e.activeItem+1)};this.prevPage=function(){f.activatePage(e.activePage-1)};this.nextPage=function(){f.activatePage(e.activePage+1)};this.toStart=function(a){a===J?v(d.min):A?(a=W(a),-1!==a&&!G&&v(k[a].offStart)):(a=j.find(a).eq(0),c.contains(j[0],a[0])&&v(b.horizontal?a.offset().left-j.offset().left:a.offset().top-j.offset().top))};this.toEnd=function(a){if(a===J)v(d.max);else if(A)a=W(a),-1!==a&&!G&&v(k[a].offEnd);else if(a=j.find(a).eq(0),c.contains(j[0],a[0])){var e=
...
ba();c!==h&&n.trigger(r+":active",[p.eq(c)]);d||(G?f.toCenter(c):ia&&(c>=e.lastItem?f.toStart(c):c<=e.firstItem?f.toEnd(c):f.cycle()))}};this.activatePage=function(a){a=H(a,0,w.length-1);v(w[a])};this.cycle=function(a){if(b.cycleBy&&b.cycleInterval&&!("items"===b.cycleBy&&!k[0]||a&&ha))B?B=clearTimeout(B):n.trigger(r+":cycleStart",[d,p,e]),B=setTimeout(function(){switch(b.cycleBy){case "items":f.activate(e.activeItem>=k.length-1?0:e.activeItem+1);break;case "pages":f.activatePage(e.activePage>=w.length-
...
(c=0>-a.detail/3);A?(a=W((G?D?e.activeItem:e.centerItem:e.firstItem)+(c?b.scrollBy:-b.scrollBy)),f[G?D?"activate":"toCenter":"toStart"](a)):v(d.dest+(c?b.scrollBy:-b.scrollBy))}});b.keyboardNav&&T.bind("keydown."+m,function(a){switch(a.which){case b.horizontal?37:38:x(a);f[b.keyboardNavByPages?"prevPage":"prev"]();break;case b.horizontal?39:40:x(a),f[b.keyboardNavByPages?"nextPage":"next"]()}});if(b.prev)O.on("click."+m,function(a){x(a);f.prev()});if(b.next)P.on("click."+m,function(a){x(a);f.next()});
...
k=u.max-u.cur;y.addClass(b.draggedClass);n.trigger(r+":moveStart",[d,p,e,"scrollbar"]);f.pause(1);T.on(da,function(a){x(a);var f=H((b.horizontal?a.clientX:a.clientY)-c,j,k);v(Math.round((h+f)/u.max*(d.max-d.min))+d.min,"handle","mouseup"===a.type);"mouseup"===a.type&&(T.off(da),y.removeClass(b.draggedClass),n.trigger(r+":moveEnd",[d,p,e,"scrollbar"]))})}});if(b.cycleBy){if(b.pauseOnHover)n.on("mouseenter."+m+" mouseleave."+m,function(a){if(!ha)f["mouseenter"===a.type?"pause":"cycle"](1)});f[b.startPaused?
"pause":"cycle"]()}}function X(c){return!isNaN(parseFloat(c))&&isFinite(c)}function H(c,b,g){return c<b?b:c>g?g:c}for(var r="sly",m=r,q=window.cancelAnimationFrame||window.cancelRequestAnimationFrame,ca=window.requestAnimationFrame,M,F=window,t=["moz","webkit","o"],ba=0,g=0,I=t.length;g<I&&!q;++g)ca=(q=F[t[g]+"CancelAnimationFrame"]||F[t[g]+"CancelRequestAnimationFrame"])&&F[t[g]+"RequestAnimationFrame"];q||(ca=function(c){var b=+new Date,g=Math.max(0,16-(b-ba));ba=b+g;return F.setTimeout(function(){c(b+
