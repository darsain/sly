...
(a.stopPropagation?a.stopPropagation():a.cancelBubble=!0)}function I(a){return!isNaN(parseFloat(a))&&isFinite(a)}var h=this,n=e(N),U=b.childrenFilter||"*",f=n.children().eq(0),w=0,j=0,c={cur:0,max:0,min:0},L=e(b.scrollBar).eq(0),m=L.length?L.children().eq(0):0,R=0,M=0,r={cur:0,max:0,min:0},Y=e(b.pagesBar),O=0,v=[],$="smart"===b.itemNav,C="forceCentered"===b.itemNav,G="centered"===b.itemNav||C,y="basic"===b.itemNav||$||G||C,l=0,k=[],d={firstItem:0,lastItem:1,centerItem:1,activeItem:-1,activePage:0,
items:0,pages:0},aa=b.scrollSource?e(b.scrollSource):n,da=b.dragSource?e(b.dragSource):n,J=e(b.prev),K=e(b.next),P=e(b.prevPage),Q=e(b.nextPage),H=0,ba=0,D=0,ha=this.reload=function(){var a=0,t=e.extend({},c);clearTimeout(H);w=b.horizontal?n.width():n.height();R=b.horizontal?L.width():L.height();j=b.horizontal?f.outerWidth():f.outerHeight();l=f.children(U);k=[];v=[];c.min=0;c.max=Math.max(j-w,0);d.items=l.length;if(y){var E=q(l,b.horizontal?"marginLeft":"marginTop"),F=q(l.slice(-1),b.horizontal?"marginRight":
...
else for(;A-w<c.max;){var ca=Math.min(c.max,A);v.push(ca);A+=w;if(A>c.max&&y&&c.max-ca<(k[k.length-1].size-a)/2){v[v.length-1]=c.max;break}}if(Y.length){for(a=0;a<v.length;a++)F+=b.pageBuilder(B++);O=e(F).bind("click."+s,function(){h.activatePage(O.index(this))}).appendTo(Y.empty())}l.unbind("."+s).bind("mouseup."+s,function(a){1===a.which&&!D&&h.activate(this)});c.cur<c.min&&g(c.min);c.cur>c.max&&g(c.max);d.pages=v.length;d.slideeSize=j;d.frameSize=w;d.sbSize=R;d.handleSize=M;z(0);W();b.cycleBy&&
(F="mouseenter."+s+" mouseleave."+s,b.pauseOnHover&&n.unbind(F).bind(F,function(a){if(!ba)h["mouseenter"===a.type?"pause":"cycle"](1)}),b.startPaused||h.cycle());n.trigger(p+":load",[e.extend({},c,{old:t}),l,d])};this.prev=function(){h.activate(d.activeItem-1)};this.next=function(){h.activate(d.activeItem+1)};this.prevPage=function(){h.activatePage(d.activePage-1)};this.nextPage=function(){h.activatePage(d.activePage+1)};this.toStart=function(a){if(y){var d=B(a);if(a===x)g(c.min,1);else if(-1!==d){if(G)return;
...
this.pause=function(a){H&&(a||(ba=!0),H=clearTimeout(H),n.trigger(p+":cyclePause",[c,l,d]))};this.toggle=function(){h[H?"pause":"cycle"]()};this.set=function(a,c){e.isPlainObject(a)?e.extend(b,a):b.hasOwnProperty(a)&&(b[a]=c)};this.destroy=function(){n.add(document).add(f).add(l).add(aa).add(m).add(J).add(K).add(P).add(Q).unbind("."+s);f.add(m).css(b.horizontal?"left":"top",0);J.add(K).removeClass(b.disabledClass);Y.empty();e.removeData(N,s)};var S=e(document),T="mousemove."+s+" mouseup."+s;b=e.extend({},
e.fn[p].defaults,b);n.css("overflow","hidden");"static"===n.css("position")&&n.css("position","relative");"static"===L.css("position")&&L.css("position","relative");f.add(m).css(e.extend({position:"absolute"},b.horizontal?{left:0}:{top:0}));ha();y?h.activate(b.startAt):g(b.startAt);z();b.scrollBy&&aa.bind("DOMMouseScroll."+s+" mousewheel."+s,function(a){if(c.min!==c.max){u(a,1);a=a.originalEvent;var e=0;a.wheelDelta&&(e=a.wheelDelta/120);a.detail&&(e=-a.detail/3);a=0>e;y?(a=B((G?C?d.activeItem:d.centerItem:
...
b.nextPage&&Q.bind("click."+s,function(a){u(a);h.nextPage()});b.dragContent&&da.bind("mousedown."+s,function(a){if(1===a.which){u(a);var d=a.clientX,h=a.clientY,k=c.cur,r=+new Date,l=a.target,j=0;f.addClass(b.draggedClass);X();S.bind(T,function(a){var A="mouseup"===a.type,n=b.horizontal?a.clientX-d:a.clientY-h,m=k-n;!j&&10<Math.abs(n)&&(j=1,console.log("asdsa",n),f.trigger(p+":dragStart",[c]));m>c.max?m=b.elasticBounds?c.max+(m-c.max)/6:c.max:m<c.min&&(m=b.elasticBounds?c.min+(m-c.min)/6:c.min);if(A&&
