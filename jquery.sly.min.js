...
{},s=B?0:t/2,d=0;d<u.length;d++){if(a>=c.max||d===u.length-1){b.activePage=u.length-1;break}if(a<=u[d]+s){b.activePage=d;break}}if(w){for(var e=d=s=!1,f=0;f<l.length;f++){!1===s&&a<=l[f].offStart&&(s=f);!1===e&&a-l[f].size/2<=l[f].offCenter&&(e=f);if(f===l.length-1||!1===d&&a<l[f+1].offEnd)d=f;if(!1!==d)break}b.firstItem=G(s)?s:0;b.centerItem=G(e)?e:b.firstItem;b.lastItem=G(d)?d:b.centerItem}return b}function U(){if(w){var a=0===d.activeItem,r=d.activeItem>=l.length-1;H.is("button,input")&&H.prop("disabled",
...
a>=u.length?u.length-1:a;i(u[a],w);z()};this.cycle=function(a,e){w&&b.cycleBy&&(e||($=!!a),a?L&&(L=clearTimeout(L),j.trigger(o+":cyclePause",[c,m,d])):L||(j.trigger(o+":cycleStart",[c,m,d]),function C(){L=setTimeout(function(){if(!D)switch(b.cycleBy){case "items":g.activate(d.activeItem>=l.length-1?0:d.activeItem+1);break;case "pages":g.activatePage(d.activePage>=u.length-1?0:d.activePage+1)}j.trigger(o+":cycle",[c,m,d]);C()},b.cycleInterval)}()))};this.destroy=function(){j.add(document).add(h).add(m).add(Z).add(p).add(H).add(I).add(O).add(P).unbind("."+
q);h.add(p).css(b.horizontal?{left:0}:{top:0});H.add(I).removeClass(b.disabledClass);W.empty();f.removeData(M,q)};var Q=f(document),R="mousemove."+q+" mouseup."+q,b=f.extend({},f.fn[o].defaults,b);"static"===j.css({overflow:"hidden"}).css("position")&&j.css({position:"relative"});"static"===J.css("position")&&J.css({position:"relative"});h.add(p).css(b.horizontal?{position:"absolute",left:0}:{position:"absolute",top:0});fa();w?g.activate(b.startAt):i(b.startAt);z();b.scrollBy&&Z.bind("DOMMouseScroll."+
q+" mousewheel."+q,function(a){if(c.min!==c.max){v(a,1);var a=a.originalEvent,e=0;a.wheelDelta&&(e=a.wheelDelta/120);a.detail&&(e=-a.detail/3);a=0>e;w?(a=E((F?B?d.activeItem:d.centerItem:d.firstItem)+(a?b.scrollBy:-b.scrollBy)),g[F?B?"activate":"toCenter":"toStart"](a)):i(c.cur+(a?b.scrollBy:-b.scrollBy));z()}});b.keyboardNav&&Q.bind("keydown."+q,function(a){switch(a.keyCode||a.which){case b.horizontal?37:38:v(a);b.keyboardNavByPages?g.prevPage():g.prev();break;case b.horizontal?39:40:v(a),b.keyboardNavByPages?
g.nextPage():g.next()}});b.prev&&H.bind("click."+q,function(a){v(a);g.prev()});b.next&&I.bind("click."+q,function(a){v(a);g.next()});b.prevPage&&O.bind("click."+q,function(a){v(a);g.prevPage()});b.nextPage&&P.bind("click."+q,function(a){v(a);g.nextPage()});b.dragContent&&S.bind("mousedown."+q,function(a){if(1===a.which){v(a);var d=a.clientX,e=a.clientY,g=c.cur,l=+new Date,k=a.target,m=0;h.addClass(b.draggedClass);V();Q.bind(R,function(a){var y="mouseup"===a.type,p=b.horizontal?a.clientX-d:a.clientY-
e,j=g-p;!m&&10<Math.abs(p)&&(m=1,h.trigger(o+":dragStart",[c]));j>c.max?j=b.elasticBounds?c.max+(j-c.max)/6:c.max:j<c.min&&(j=b.elasticBounds?c.min+(j-c.min)/6:c.min);if(y){Q.unbind(R);h.removeClass(b.draggedClass);var n=+new Date-l,n=300>n?Math.ceil(Math.pow(6/(n/300),2)*Math.abs(p)/120):0,j=j+(0<p?-n:n)}m&&(v(a),k&&(f(k).bind("click."+q,function da(a){v(a,true);f(this).unbind("click."+q,da)}),k=0),D=!y,i(j,y,y?b.speed:0),z(y?null:0),m&&h.trigger(o+":drag",[c]),y&&h.trigger(o+":dragEnd",[c]))})}});
p&&b.dragHandle&&p.bind("mousedown."+q,function(a){if(1===a.which){v(a);var d=a.clientX,f=a.clientY,g=e.cur,k=-e.cur,l=e.max-e.cur,j=0;p.addClass(b.draggedClass);V();Q.bind(R,function(a){v(a);var h="mouseup"===a.type,a=b.horizontal?a.clientX-d:a.clientY-f,m=g+a,n=+new Date;D=!h;h&&(Q.unbind(R),p.removeClass(b.draggedClass));if(a<l+5&&a>k-5||h){e.cur=m>e.max?e.max:m<e.min?e.min:m;p.stop().css(b.horizontal?{left:e.cur+"px"}:{top:e.cur+"px"});j||p.trigger(o+":dragStart",[e]);p.trigger(o+":drag",[e]);
h&&p.trigger(o+":dragEnd",[e]);if(j<=n||h||a>l||a<k)j=n+50,i(Math.round(e.cur/e.max*(c.max-c.min))+c.min,h,h?b.speed:50);X()}})}})}var o="sly",q="plugin_"+o;f.fn[o]=function(o,b){var i=!1,z,x=[];"undefined"!==typeof o&&!f.isPlainObject(o)&&(i=!1===o?"destroy":o,z=arguments,Array.prototype.shift.call(z));this.each(function(b,E){var n=f.data(E,q);n&&i?n[i]&&n[i].apply(n,z):!n&&!i&&(n=f.data(E,q,new S(E,o)));x.push(n)});return b&&!i?1<x.length?x:x[0]:this};f.fn[o].defaults={horizontal:0,itemNav:0,scrollBar:null,
dynamicHandle:1,dragHandle:1,minHandleSize:50,pagesBar:null,pageBuilder:function(f){return"<li>"+(f+1)+"</li>"},prev:null,next:null,prevPage:null,nextPage:null,cycleBy:0,cycleInterval:5E3,pauseOnHover:1,startPaused:0,scrollBy:0,dragContent:0,elasticBounds:0,speed:300,easing:"swing",scrollSource:null,dragSource:null,startAt:0,keyboardNav:0,keyboardNavByPages:0,draggedClass:"dragged",activeClass:"active",disabledClass:"disabled"}})(jQuery);
