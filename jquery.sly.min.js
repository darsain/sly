...
n)};(function(){var b=i(document),j="mousemove."+n+" mouseup."+n;"static"===r.css({overflow:"hidden"}).css("position")&&r.css({position:"relative"});"static"===I.css("position")&&I.css({position:"relative"});f.add(p).css(a.horizontal?{position:"absolute",left:0}:{position:"absolute",top:0});Q();o?h.activate(a.startAt):q(a.startAt);x();a.scrollBy&&X.bind("DOMMouseScroll."+n+" mousewheel."+n,function(b){u(b,1);var b=b.originalEvent,f=0;b.wheelDelta&&(f=b.wheelDelta/120);b.detail&&(f=-b.detail/3);b=
0>f;o?(b=y((E?z?c.activeItem:c.centerItem:c.firstItem)+(b?a.scrollBy:-a.scrollBy)),h[E?z?"activate":"toCenter":"toStart"](b)):q(d.cur+(b?a.scrollBy:-a.scrollBy));x()});a.keyboardNav&&b.bind("keydown."+n,function(b){switch(b.keyCode||b.which){case a.horizontal?37:38:u(b);a.keyboardNavByPages?h.prevPage():h.prev();break;case a.horizontal?39:40:u(b),a.keyboardNavByPages?h.nextPage():h.next()}});a.prev&&G.bind("click."+n,function(a){u(a);h.prev()});a.next&&H.bind("click."+n,function(a){u(a);h.next()});
a.prevPage&&N.bind("click."+n,function(a){u(a);h.prevPage()});a.nextPage&&O.bind("click."+n,function(a){u(a);h.nextPage()});a.dragContent&&f.bind("mousedown."+n,function(c){if(1===c.which){u(c);var g=c.clientX,h=c.clientY,l=d.cur,m=+new Date,p=c.target,r=0;f.addClass(a.draggedClass);T();b.bind(j,function(c){var e="mouseup"===c.type,s=a.horizontal?c.clientX-g:c.clientY-h,k=l-s;!r&&10<Math.abs(s)&&(r=1,f.trigger(t+":dragStart",[d]));k>d.max?k=a.elasticBounds?d.max+(k-d.max)/6:d.max:k<d.min&&(k=a.elasticBounds?
d.min+(k-d.min)/6:d.min);if(e){b.unbind(j);f.removeClass(a.draggedClass);var o=+new Date-m,o=300>o?Math.ceil(Math.pow(6/(o/300),2)*Math.abs(s)/120):0,k=k+(0<s?-o:o)}r&&(u(c),p&&(i(p).bind("click."+n,function $(a){u(a,true);i(this).unbind("click."+n,$)}),p=0),A=!e,q(k,e,e?a.speed:0),x(e?null:0),r&&f.trigger(t+":drag",[d]),e&&f.trigger(t+":dragEnd",[d]))})}});p&&a.dragHandle&&p.bind("mousedown."+n,function(c){if(1===c.which){u(c);var f=c.clientX,h=c.clientY,i=g.cur,k=-g.cur,l=g.max-g.cur,m=0;p.addClass(a.draggedClass);
T();b.bind(j,function(c){u(c);var e="mouseup"===c.type,c=a.horizontal?c.clientX-f:c.clientY-h,n=i+c,o=+new Date;A=!e;e&&(b.unbind(j),p.removeClass(a.draggedClass));if(c<l+5&&c>k-5||e){g.cur=n>g.max?g.max:n<g.min?g.min:n;p.stop().css(a.horizontal?{left:g.cur+"px"}:{top:g.cur+"px"});m||p.trigger(t+":dragStart",[g]);p.trigger(t+":drag",[g]);e&&p.trigger(t+":dragEnd",[g]);if(m<=o||e||c>l||c<k)m=o+50,q(Math.round(g.cur/g.max*(d.max-d.min))+d.min,e,e?a.speed:50);V()}})}})})()}var t="sly",n="plugin_"+t;
i.fn[t]=function(w){var a=!1,q=Array.prototype.slice.call(arguments,1),x={};i.isPlainObject(w)?x=i.extend({},i.fn[t].defaults,w):a=!1===w?"destroy":w;return this.each(function(t,w){var y=i.data(w,n)||i.data(w,n,new Q(w,x));i.isFunction(y[a])&&y[a].apply(y,q)})};i.fn[t].defaults={horizontal:0,itemNav:0,scrollBar:null,dynamicHandle:1,dragHandle:1,pagesBar:null,pageBuilder:function(i){return"<li>"+(i+1)+"</li>"},prev:null,next:null,prevPage:null,nextPage:null,cycleBy:0,cycleInterval:5E3,pauseOnHover:1,
startPaused:0,scrollBy:0,dragContent:0,elasticBounds:0,speed:300,easing:"swing",scrollSource:null,startAt:0,keyboardNav:0,keyboardNavByPages:0,draggedClass:"dragged",activeClass:"active",disabledClass:"disabled"}})(jQuery);
