/*! Sly v1.0.0-rc.1 17-Jan-2013 | https://github.com/Darsain/sly */
...
ga=g(k).appendTo(ba.empty())}}x(J(d.dest,d.min,d.max));e.pages=t.length;e.slideeSize=n;e.frameSize=z;e.sbSize=ja;e.handleSize=R;B("load")};c.getPos=function(a){if(a===O)return d;if(H)return a=wa(a),-1!==a?l[a]:!1;var c=r.find(a).eq(0);return c[0]?(a=b.horizontal?c.offset().left-r.offset().left:c.offset().top-r.offset().top,c=c[b.horizontal?"outerWidth":"outerHeight"](),{start:a,center:a-z/2+c/2,end:a-z+c}):!1};c.getRel=function(){return e};c.prev=function(){c.activate(e.activeItem-1)};c.next=function(){c.activate(e.activeItem+
1)};c.prevPage=function(){c.activatePage(e.activePage-1)};c.nextPage=function(){c.activatePage(e.activePage+1)};c.slideBy=function(a){x(d.dest+a)};c.slideTo=function(a){x(a)};c.toStart=function(a){a===O?x(d.min):Q||(a=c.getPos(a))&&x(a.start)};c.toEnd=function(a){a===O?x(d.max):Q||(a=c.getPos(a))&&x(a.end)};c.toCenter=function(a){a===O?x(Math.round(d.max/2+d.min/2)):(a=c.getPos(a))&&x(a.center)};c.activate=function(a,d){if(H&&a!==O){var h=wa(a),f=e.activeItem;e.activeItem=h;s.eq(f).removeClass(b.activeClass);
s.eq(h).addClass(b.activeClass);ca();h!==f&&B("active",h);d||(Q?c.toCenter(h):Ba&&(h>=e.lastItem?c.toStart(h):h<=e.firstItem?c.toEnd(h):c.cycle()))}};c.activatePage=function(a){a=J(a,0,t.length-1);x(t[a]);B("activePage",a)};c.cycle=function(a){if(b.cycleBy&&b.cycleInterval&&!("items"===b.cycleBy&&!l[0]||a&&fa))fa=0,M?M=clearTimeout(M):B("cycleStart"),M=setTimeout(function(){switch(b.cycleBy){case "items":c.activate(e.activeItem>=l.length-1?0:e.activeItem+1);break;case "pages":c.activatePage(e.activePage>=
t.length-1?0:e.activePage+1)}B("cycle")},b.cycleInterval)};c.pause=function(a){a||(fa=!0);M&&(M=clearTimeout(M),B("cyclePause"))};c.toggle=function(){c[M?"pause":"cycle"]()};c.set=function(a,c){g.isPlainObject(a)?g.extend(b,a):b.hasOwnProperty(a)&&(b[a]=c)};c.on=function(a,b){if("object"===typeof a)for(var d in a){if(a.hasOwnProperty(d))c.on(d,a[d])}else if(b instanceof Array){d=0;for(var e=b.length;d<e;d++)c.on(a,b[d])}else if("function"===typeof b){u[a]=u[a]||[];d=0;for(e=u[a].length;d<e;d++)if(u[a][d]===
b)return;u[a].push(b)}};c.off=function(a,b){if(u[a])if(b instanceof Array)for(var d=0,e=b.length;d<e;d++)c.off(a,b[d]);else if("function"===typeof b){d=0;for(e=u[a].length;d<e;d++)u[a][d]===b&&u[a].splice(d,1)}else u[a].length=0};c.destroy=function(){na.add(r).add(Ca).add(y).add(E).add(ba).add(Y).add(Z).add($).add(aa).unbind("."+j);Y.add(Z).add($).add(aa).removeClass(b.disabledClass);s&&s.eq(e.activeItem).removeClass(b.activeClass);ba.empty();C||(D.unbind("."+j),r.add(y).css(L||(b.horizontal?"left":
"top"),L?"none":0),g.removeData(f,j))};c.init=function(){c.on(pa);C||(D.css("overflow","hidden"),"static"===D.css("position")&&D.css("position","relative"),r.add(y).css(g.extend({position:"absolute"},b.horizontal?{left:0}:{top:0})));"static"===E.css("position")&&E.css("position","relative");Ga();H?c.activate(b.startAt):x(b.startAt);b.scrollBy&&Ca.bind("DOMMouseScroll."+j+" mousewheel."+j,function(a){if(d.min!==d.max){A(a,1);a=a.originalEvent;var f=0;a.wheelDelta&&(f=0>a.wheelDelta/120);a.detail&&
(f=0>-a.detail/3);H?(a=wa((Q?I?e.activeItem:e.centerItem:e.firstItem)+(f?b.scrollBy:-b.scrollBy)),c[Q?I?"activate":"toCenter":"toStart"](a)):c.slideBy(f?b.scrollBy:-b.scrollBy)}});if(b.clickBar&&E[0])E.on("click."+j,function(a){1>=a.which&&(A(a),x(ka((b.horizontal?a.clientX-E.offset().left:a.clientY-E.offset().top)-R/2)))});b.keyboardNavBy&&na.bind("keydown."+j,function(a){switch(a.which){case b.horizontal?37:38:A(a);c["pages"===b.keyboardNavBy?"prevPage":"prev"]();break;case b.horizontal?39:40:A(a),
c["pages"===b.keyboardNavBy?"nextPage":"next"]()}});if(b.prev)Y.on("click."+j,function(a){A(a);c.prev()});if(b.next)Z.on("click."+j,function(a){A(a);c.next()});if(b.prevPage)$.on("click."+j,function(a){A(a);c.prevPage()});if(b.nextPage)aa.on("click."+j,function(a){A(a);c.nextPage()});r.on("click."+j,"*",function(a){1>=a.which&&(this.parentNode===a.delegateTarget&&!oa)&&c.activate(this);oa=0});if(ba[0])ba.on("click."+j,"*",function(){c.activatePage(ga.index(this))});if(b.dragging)Fa.on(Da,{src:"slidee"},
xa);if(b.dragHandle&&y)y.on(Da,{src:"handle"},xa);if(b.cycleBy&&!C){if(b.pauseOnHover)D.on("mouseenter."+j+" mouseleave."+j,function(a){if(!fa)c["mouseenter"===a.type?"pause":"cycle"](1)});c[b.startPaused?"pause":"cycle"]()}return c}}function A(f,b){f=f||F.event;f.preventDefault?f.preventDefault():f.returnValue=!1;b&&(f.stopPropagation?f.stopPropagation():f.cancelBubble=!0)}function W(f){return!isNaN(parseFloat(f))&&isFinite(f)}function J(f,b,g){return f<b?b:f>g?g:f}for(var ia="sly",j=ia,m=F.cancelAnimationFrame||
F.cancelRequestAnimationFrame,ma=F.requestAnimationFrame,L,N=window,q=["moz","webkit","o"],ka=0,k=0,S=q.length;k<S&&!m;++k)ma=(m=N[q[k]+"CancelAnimationFrame"]||N[q[k]+"CancelRequestAnimationFrame"])&&N[q[k]+"RequestAnimationFrame"];m||(ma=function(f){var b=+new Date,g=Math.max(0,16-(b-ka));ka=b+g;return N.setTimeout(function(){f(b+g)},g)},m=function(f){clearTimeout(f)});m=["transform","webkitTransform","msTransform"];q=document.createElement("div");k=0;for(S=m.length;k<S&&!L;k++)q.style[m[k]]!==
O&&(L=m[k]);F.Sly=ca;g.fn[ia]=function(f,b){var k,m;if(!g.isPlainObject(f)){if("string"===typeof f||!1===f)k=!1===f?"destroy":f,m=arguments,Array.prototype.shift.call(m);f={}}return this.each(function(A,w){var q=g.data(w,j);!q&&!k?g.data(w,j,(new ca(w,f,b)).init()):q&&k&&q[k]&&q[k].apply(q,m)})};g.fn[ia].defaults={horizontal:0,itemNav:null,scrollBar:null,dragHandle:0,dynamicHandle:0,minHandleSize:50,clickBar:0,syncFactor:0.5,pagesBar:null,pageBuilder:function(f){return"<li>"+(f+1)+"</li>"},prev:null,
next:null,prevPage:null,nextPage:null,cycleBy:null,cycleInterval:5E3,pauseOnHover:0,startPaused:0,scrollBy:0,dragging:0,elasticBounds:0,speed:0,easing:"swing",scrollSource:null,dragSource:null,startAt:0,keyboardNavBy:0,domEvents:0,draggedClass:"dragged",activeClass:"active",disabledClass:"disabled"}})(jQuery,window);
